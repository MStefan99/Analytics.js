...
(() => {
		...
		.then((json) => {
			...
			for (let i = 0; i < 30; ++i) {
				if (json.sessions[startTime + sessionLength * i]) {
					const mapScale = map.bind(
						this,
						Object.keys(json.sessions).reduce((a, k) => Math.max(a, json.sessions[k]), 0),
						svgRect.height
					);
			...
			for (const page of Object.keys(json.pages)
				...
				if (pageAddress.length > 40) {
					pageAddress =
						pageAddress.substr(0, 15) + '...' + pageAddress.substr(pageAddress.length - 5, 5);
				}
	...
	fetch('//' + analyticsLocation + '/api/stats/today/' + websiteID, {})
		...
		.then((json) => {
			...
			sessionDurationElement.text(
				Math.floor((json.avgDuration / 60 / 1000) % 60) +
					'm ' +
					Math.floor((json.avgDuration / 1000) % 60) +
					's'
			);
		});
})();
