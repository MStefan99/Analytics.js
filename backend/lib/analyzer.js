...
module.exports = {
		...
		return new Promise((resolve) =>
			configurer(
				...
				for (const userID of Object.keys(data)) {
					...
					const requests = data[userID].requests.filter((r) =>
						Date.now() - r.time < this.dayLength
					);
					for (let i = 0; i < requests.length; ++i) {
						...
						for (
							let j = i + 1;
							j < requests.length &&
							requests[j].time - requests[j - 1].time <
								this.sessionLength;
							++j
						) {
							session.duration += requests[j].time -
								requests[j - 1].time;
			...
			configurer(
				...
				for (const userID of Object.keys(data)) {
					const lastRequest = data[userID].requests.length
						? data[userID]
							.requests[data[userID].requests.length - 1]
						: undefined;
					const requests = data[userID].requests.filter((r) =>
						Date.now() - r.time < this.sessionLength
					);
					...
					for (const request of requests) {
						const sessionTime = request.time -
							(request.time % this.realtimeLength);

						audience.pages[request.url] = 1 +
							(audience.pages[request.url] || 0);
						audience.sessions[sessionTime] = 1 +
							(audience.sessions[sessionTime] || 0);

						if (request.referrer !== undefined) {
							audience.referrers[request.referrer] = 1 +
								(audience.referrers[request.referrer] || 0);
						}
					}
				}
	...
	historyAudience: function (websiteID) {
		...
		return new Promise((resolve) =>
			configurer(path.resolve('../../data/hits', websiteID)).load().then(
				(data) => {
					let totalTime = 0;
					for (const userID of Object.keys(data)) {
						...
						for (let i = 0; i < requests.length; ++i) {
							...
							const day = session.time -
								(session.time % this.dayLength);
							if (!archive[day]) {
								archive[day] = {
									sessions: 0,
									urls: {},
							...
							archive[day].bounceRate = bounced /
								archive[day].sessions;
							archive[day].avgDuration = totalTime /
								archive[day].sessions;

							for (const request of session.requests) {
								archive[day].urls[request.url] = 1 +
									(archive[day].urls[request.url] || 0);

								if (request.referrer) {
									archive[day].referrers[request.referrer] =
										1 +
										(archive[day]
											.referrers[request.referrer] || 0);
								}
							}
						}
					}
